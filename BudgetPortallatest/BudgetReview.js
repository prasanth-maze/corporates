...
// Common For All SelectBox
	...
	// Element Group Ajax
	 ...
	 $("#terriotry").on("change",function(event) { 
	      ...
	      if($("#terriotry option:selected").length>0){
	        FilterAction["terr[]"]=$(this).val(); 
	      }else{
	      	alert('Select atleat one terriotry');
	      	return false;
	      }
	     	genFilter(FilterAction);
    });
...
$("body").on("submit",".filter-form",function(event){
	 	...
	 	fsdata = sdata;
		if(CE==null){
			fsdata['Action'] = 'CEwisebudget'
	 	}else{

	 	}
	 	filterArr.push(fsdata);
