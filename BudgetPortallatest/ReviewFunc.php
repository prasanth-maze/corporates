<?php
	...
	function BussDivWise($getdata){
		...
		$costElement = implode("','",$getdata['costElement']);
		 ...
		 $sqlce = "SELECT CostElement FROM ".$costcentrelmtbl." WHERE CostElementName IN('".$costElement."') GROUP BY CostElement  ";
		 //echo $sqlce;exit;
		$res = sqlsrv_query($conn,$sqlce,array(), array( "Scrollable" => 'static' ));
		      $costElement = array();
		      while($row = sqlsrv_fetch_array($res,SQLSRV_FETCH_ASSOC)){ 
		      		//p($row,'e');
