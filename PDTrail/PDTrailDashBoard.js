       ...
       currentRequest =  $.ajax({
          ...
          success:function(res){
            ...
            if(res.hasOwnProperty('TrendChartData'))
              ...
              data = data+"&filter=SOWING"
              genReport(data,"sowingRestbl","SOWING");
      ...
      var totnt = 0;
       ...
       if(locSeriousdata.hasOwnProperty('series1')){
                    ...
                    series1SOWINGobj.push(series1SOWINGobj1[ri]);
                     if(!categoriesarr.includes(name)){
                      ...
                      if(!ntarr.hasOwnProperty(NID)){
                          ...
                          ntarr[NID]=ntarr[NID]+cobj.nt;
                          hyarr[NID]=hyarr[NID]+cobj.hc;
                           totnt=totnt+cobj.nt;
                      }
          ...
          if(series1.hasOwnProperty('STAGE_120_130')){
           ...
           if(series1.hasOwnProperty('STAGE_150_160')){
              series1STAGE_150_160obj1 = series1.STAGE_150_160;
              console.log(series1STAGE_150_160obj1);
               for(var ri in series1STAGE_150_160obj1){ 
                    var cobj = series1STAGE_150_160obj1[ri];
                    var name = cobj.name;
                    var NID = cobj.NID;
                      series1STAGE_150_160obj.push(series1STAGE_150_160obj1[ri]);
                     if(!categoriesarr.includes(name)){
                        categoriesidarr.push(NID);
                        categoriesarr.push(name);
       ...
       for(var cid in categoriesidarr){
          ...
          ntarr1.push(ntarr[cfid]);
          hcarr1.push(hyarr[cfid]);
       }
...
//          alert('You clicked on '+this.textContent);*/
   ...
   /* function genProductWiseChart(productWisesData){
    //console.log(productWisesData);
       ...
       if(series1.hasOwnProperty('STAGE_120_130')){
        ...
        if(series1.hasOwnProperty('STAGE_150_160')){
          series1STAGE_150_160obj = series1.STAGE_150_160;
       }
 ...
 function genProductWiseChart(productWisesData){
   // console.log(productWisesData);
       ...
       var categoriesarr=[];
       if(series1.hasOwnProperty('SOWING')){
          series1SOWIN = series1.SOWING;
          series1SOWIN.forEach(function(cdata){
              if(!categoriesarr.includes(cdata.name)){
                categoriesarr.push(cdata.name)
              }
               delete cdata.name;
              series1SOWINGobj.push(cdata);
          });
       }

        if(series1.hasOwnProperty('STAGE_70_80')){
         var series1STAGE_70_80 = series1.STAGE_70_80;
          series1STAGE_70_80.forEach(function(cdata){
              if(!categoriesarr.includes(cdata.name)){
                categoriesarr.push(cdata.name);                
              }
              delete cdata.name;
              series1STAGE_70_80obj.push(cdata);
          });
       }
      

       if(series1.hasOwnProperty('STAGE_120_130')){
          var series1STAGE_120_130 = series1.STAGE_120_130;
          series1STAGE_120_130.forEach(function(cdata){
              if(!categoriesarr.includes(cdata.name)){
                categoriesarr.push(cdata.name);                
              }
              delete cdata.name;
              series1STAGE_120_130obj.push(cdata);
          });
       }

        if(series1.hasOwnProperty('STAGE_150_160')){
          var series1STAGE_150_160 = series1.STAGE_150_160;
          series1STAGE_150_160.forEach(function(cdata){
              if(!categoriesarr.includes(cdata.name)){
                categoriesarr.push(cdata.name);                
              }
              delete cdata.name;
              series1STAGE_150_160obj.push(cdata);
          });
       }

       if(series1.hasOwnProperty('STAGE_PICKING_YIELD')){
          var series1STAGE_PY = series1.STAGE_PICKING_YIELD;
          series1STAGE_PY.forEach(function(cdata){
              if(!categoriesarr.includes(cdata.name)){
                categoriesarr.push(cdata.name);                
              }
              delete cdata.name;
              series1STAGE_PYobj.push(cdata);
          });
       }

       if(series1.hasOwnProperty('STAGE_CLOSED')){
           var series1STAGE_CLOSED = series1.STAGE_CLOSED;
          series1STAGE_CLOSED.forEach(function(cdata){
              if(!categoriesarr.includes(cdata.name)){
                categoriesarr.push(cdata.name);                
              }
              delete cdata.name;
              series1STAGE_CLOSEDobj.push(cdata);
          });
       }

        

        pcchart = new Highcharts.Chart({
            chart: {
                   ...
                   cursor: 'pointer',
                    ...
                    point: {
                        events: {
                          click: function() {
                            var act = this.act;
                            var pname = this.category;
                            ddclick = ddclick+1;
                            $("#getCReport").val(this.act);
                            if(CP=="PRODUCT"){
                              $(".productSelect option[value='"+this.category+"']").attr("selected","selected");
                              $(".productSelect").change();
                            }
                            if(CP=="HYBRID"){
                              $(".hybridsSelect option[value='"+this.category+"']").attr("selected","selected");
                            }
                             $(".filter-form").submit();
                          }
                        }
        ...
        pcchart.xAxis[0].labelGroup.element.childNodes.forEach(function(label)
      {
...
var coldef = {
    ...
    function genReport(filterData,tbl='sowingRestbl',resfor='SOWING'){
