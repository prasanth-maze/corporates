...
if(isset($_REQUEST['Advid']) && isset($_REQUEST['AdvPayUniqueId'])){
    $adv_id = $_REQUEST['Advid']; 
    $AdvPayUniqueId = $_REQUEST['AdvPayUniqueId']; 
}else{ 
    $adv_id = 0;
    $AdvPayUniqueId = 0;
}
if(isset($_REQUEST['submit'])){
  ...
  $uni_idd = $req_detd['res_id'];
      ...
      for($i=0,$j=0;$i<sizeof($adv_amt_id);$i++,$j++){
        ...
        $inserta = sqlsrv_query($conn,"UPDATE ANP_Advance_Payment SET DocNo='$doc_no[$i]',DocDate='$docc_date' WHERE Id = '$adv_amt_id[$i]'" ); 
     }
        ...
        <table class="table">
          ...
          <tbody>                    
            <?php   $i = $tot_adv_amt = $tot_app_amt  = $tot_paid_amt = $tot_bal_amt = 0;
              ...
              WHERE ANP_Advance_Amount.CurrentStatus=1 AND  ANP_Advance_Amount.AdvId='$adv_id' AND ANP_Advance_Payment.AdvPayUniqueId='$AdvPayUniqueId' ");  
              while($fetch_activity_det = sqlsrv_fetch_array($view_activity)){
                ?>
                <tr>
                    <td><?php echo ++$i;?><input type="hidden" name="adv_amt_id[]" value="<?php echo $fetch_activity_det['AdvPayId']; ?>"></td>
