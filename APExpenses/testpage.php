...
if($_SESSION['Dcode'] == 'ZM'){
  ...
  $divsion = sqlsrv_query($conn,"SELECT DISTINCT ZONEID,ZONENAME  FROM RASI_ZONETABLE WHERE DBMID='".$_SESSION['EmpID']."'");
    ...
    $sql ="SELECT REGIONID,EMAIL FROM  RASI_REGIONTABLE WHERE RBMID='".$_SESSION['EmpID']."'";
    $res = sqlsrv_query($conn,$sql);
    $row_count = sqlsrv_fetch_array($res);
    $req_id     = $row_count['REGIONID'];
    $user_email = $row_count['EMAIL'];
    ...
    $email ="SELECT EMAIL FROM  RASI_ZONETABLE WHERE ZONEID='".$zone_ids."' AND EMAIL != ''";
    $resa = sqlsrv_query($conn,$email);
    while($RMlevel=sqlsrv_fetch_array($resa))
    {
        $allrm_emails = $RMlevel['EMAIL'];
        $up_level_email_ids[] = $allrm_emails;
    }
    print_r($up_level_email_ids);
}elseif($_SESSION['Dcode'] == 'TM'){
    $tztable =sqlsrv_query($conn,"SELECT TOP 1 TMID,EMAIL FROM RASI_TMTABLE WHERE EMPLID='".$_SESSION['EmpID']."'");
    $row_tm = sqlsrv_fetch_array($tztable);
    $TMID_id = $row_tm['TMID'];
    $user_email = $row_tm['EMAIL'];
    ...
    $email ="SELECT EMAIL FROM  RASI_REGIONTABLE WHERE REGIONID='".$region_ids."'";
    $resa = sqlsrv_query($conn,$email);
    while($RMlevel=sqlsrv_fetch_array($resa))
    {
      if($RMlevel['EMAIL'] !=''){
        $allrm_emails = 'manoj_p@mazenetsolution.com';
        // $allrm_emails = $RMlevel['EMAIL'];
        $emails[] = $allrm_emails;
      }
    }
    ...
    if($insert){
      ...
      if(sizeof($crop_id) == $j){
        ...
        /*  */
            ...
            $mail->setFrom($name);
            foreach($up_level_email_ids as $key => $val){   // To Mail ids
