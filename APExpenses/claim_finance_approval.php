...
if(isset($_REQUEST['submit'])){
  ...
  $adv_approve_amt = $_REQUEST['adv_approve_amt'];
    ...
    for($i=0,$j=0;$i<sizeof($set_claim_id);$i++,$j++){
      $inserta = sqlsrv_query($conn,"UPDATE ANP_Settlements_Claim SET VerifiedAmount='$approve_amt[$i]',VerifiedBy='$approved_by',VerifiedDate='$approved_at' WHERE Id=$set_claim_id[$i]" ); 
    }
    if(sizeof($set_claim_id) == $j){
      for($ii=0,$jj=0;$ii<sizeof($set_AdvRef_id);$ii++,$jj++){
        $inserta = sqlsrv_query($conn,"UPDATE ANP_Settlements_AdvanceRef SET VerifiedAmount='$adv_approve_amt[$ii]',VerifiedBy='$approved_by',VerifiedDate='$approved_at' WHERE Id = $set_AdvRef_id[$ii]" ); 
      }
      if(sizeof($set_AdvRef_id) == $jj){
        $tot_appr_amt = array_sum($approve_amt);
        /* echo "<script type='text/javascript'>alert('Thank You </br> Claim ID Is $claim_id </br> Claim Request of Rs. $tot_appr_amt has been  Verified')</script>";
        echo '<script type="text/javascript">window.location.replace("view_claim_finance_verify.php");</script>'; */
        echo "<script>window.location='claim_finance_approval.php?claim_id=".$claim_id."'</script>";

      }else{
        echo '<script type="text/javascript">window.location.replace("claim_finance_approval.php?sts=fail");</script>';
      }
    }else {
      echo '<script type="text/javascript">window.location.replace("claim_finance_approval.php?sts=fail");</script>';
  } 
      ...
      <div class="fullscreen-container">
        <div id="popdiv">
          ...
          <h3>  
              ...
              Claim Request has been verified.</br>
          </h3>
        ...
        <script>
            $( document ).ready(function() {
              ...
              $("#but2").click(function() {
                window.location='view_claim_finance_verify.php';
...
<!--             <th class="right">Base Amt.</th>
            ...
            <th class="right">Verify Amt.</th>
          </thead>
          <tbody>                    
            <?php   $i = $tot_base_amt = $tot_gst_amt = $tot_settle_amt = $tot_approved_amt = 0;
              ...
              ,ANP_Settlements_Claim.ApprovedAmount As ApprovedAmt
              ...
              while($fetch_activity_det = sqlsrv_fetch_array($view_activity)){
                ...
                <tr>
                    ...
                    <td class ="right"><?php $tot_settle_amt = $tot_settle_amt + $fetch_activity_det['SubmittedAmt']; echo $fetch_activity_det['SubmittedAmt'] ;?></td>
                    <td class ="max_amount right"><?php $tot_approved_amt = $tot_approved_amt + $fetch_activity_det['ApprovedAmt']; echo $fetch_activity_det['ApprovedAmt'] ;?></td>
                    <td ><div>
                    <input type="hidden" class="form-control" name="set_claim_id[]" value="<?php echo $fetch_activity_det['set_claim_id'];?>">
                    <input type="text" class="form-control right only_numbers required_for_valid entering_amt" name="approve_amt[]" value="<?php echo $fetch_activity_det['ApprovedAmt'];?>">
                    </div></td>
                </tr>
                <?php
                }
              ...
              <tr class="bold_total">
                  ...
                  <td align="right"><?php echo number_format((float)$tot_approved_amt, 2, '.', ''); ?></td>
                  <td class="entered_total_amt right"><?php echo number_format((float)$tot_approved_amt, 2, '.', ''); ?></td>
              </tr>
            </tbody>
          </table>
              </div>
              ...
              <table class="table" id="advance_taken">
                ...
                <tbody>       
            <?php
            $tot_advpaid_amt = $tot_advclaimpaid_amt = $tot_balclaim_amt = $tot_settle_amt = $tot_appr_amt = 0;
              ...
              ,ANP_Settlements_AdvanceRef.ApprovedAmount
              ...
              while($fetch_adv_det = sqlsrv_fetch_array($view_adv_adj)){
                ...
                <tr>
                    ...
                    <td class="right "><?php $tot_settle_amt=$tot_settle_amt+$fetch_adv_det['SettledAmount']; echo $fetch_adv_det['SettledAmount']; ?></td>
                    <td class="right max_amount1"><?php $tot_appr_amt=$tot_appr_amt+$fetch_adv_det['ApprovedAmount']; echo $fetch_adv_det['ApprovedAmount']; ?></td>
                    <td ><div >
                    <input type="hidden"  class="form-control" name="set_AdvRef_id[]" value="<?php echo $fetch_adv_det['set_AdvRef_id'];?>">
                    <input type="text"  class="form-control right only_numbers required_for_valid entering_amt1" name="adv_approve_amt[]" value="<?php echo $fetch_adv_det['ApprovedAmount'];?>">
                    </div></td>
                </tr>
                <?php
                }
              ...
              <tr class="bold_total">
                  ...
                  <td class="right"><?php echo number_format((float)$tot_appr_amt, 2, '.', ''); ?></td>
                ...
                <div class="col-xs-12" style="float: right;">
                    <a href="view_claim_finance_verify.php" class="btn btn-danger">Back</a>
