...
if(isset($_REQUEST['submit'])){
  ...
  $insert = "INSERT INTO ANP_Advance(ReqIdPre,ReqId,ReqDate,ReqDivisionId,ReqDivisionName,ReqRegionId,ReqRegionName,ReqTeritoryId,ReqTeritoryName,AdvanceTo,CreatedBy,CreatedAt ) VALUES ('$request_id','$request_id','$request_date','$division_id','$division_name','$region_id','$region_name','$teritory_id','$teritory_name','$emp_id','$created_by','$created_at')";
  // $insert = sqlsrv_query($conn,"INSERT INTO ANP_Advance(ReqIdPre,ReqId,ReqDate,ReqDivisionId,ReqDivisionName,ReqRegionId,ReqRegionName,ReqTeritoryId,ReqTeritoryName,AdvanceTo,CreatedBy,CreatedAt ) VALUES ('$request_id','$request_id','$request_date','$division_id','$division_name','$region_id','$region_name','$teritory_id','$teritory_name','$emp_id','$created_by','$created_at')");
  if($insert){
    ...
    for($i=0,$j=0;$i<sizeof($crop_id);$i++,$j++){
         $inserta = "INSERT INTO ANP_Advance_Amount(AdvId,CropId,ActivityId,SubActivityId,AdvAmount,CreatedBy,CreatedAt) VALUES ('$adv_ids','$crop_id[$i]','$activity_id[$i]','$sub_activity_id[$i]','$avt_amt[$i]','$created_by','$created_at')"; 
...
echo "<script>window.location='adv_test.php?request_id=".$request_id."'</script>";
      ...
      /*  */
        ...
        $message.= " <tr>
          ...
          if($mail->Send())
          {
            ...
            echo '<script type="text/javascript">
              window.location.replace("view_advance_request.php");
              </script>';
          }else {      
            echo '<script type="text/javascript">
        ...
        echo "F";
        // echo '<script type="text/javascript"> window.location.replace("view_advance_request.php?sts=fail");</script>';
    }
  }else{
    echo "F";
/*     echo '<script type="text/javascript">
...
<?php if(isset($_REQUEST['request_id'])){  ?>
        <div class="fullscreen-container">
          <div id="popdiv">
            <h1> Thank Yoyu </h1>
            <h3>  
                Request ID ADV/7200/3444/001 </br>
                Claim Request ID RS 4000 has verfied.</br>
            </h3>
          ...
          <table class="table">
            ...
            <tbody>
              <tr>
                ...
                <td><div>
                  <input type="text"  class="form-control right only_numbers required_for_valid" name="avt_amt[]"/>
                </td></div>
                <td>
